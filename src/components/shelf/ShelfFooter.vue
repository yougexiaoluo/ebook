  ...
  export default {
    ...
    methods: {
      onTabClick (item) {
        ...
        switch (item.index) {
          case 1:
            this.showPrivate() // 私密模式
          break
          case 2:
            this.showDownload() // 离线缓存
          break
          case 3:
            this.dialog().show() // 移动到某分组
          break
          case 4:
            this.showRemove() // 从书架中移除
          break
          default:
          break
        }
      ...
      removeSelected () {
        if (this.currentType === 1) {
          ...
          // 移除分组中的书籍(移出书架)
          this.shelfSelected.forEach(selected => {
            this.shelfCategory.itemList = this.shelfCategory.itemList.filter(book => book !== selected)
          })
        }
